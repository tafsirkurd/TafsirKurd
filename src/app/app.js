...
function _loadGencineScripts(cb) {
  ...
  function _check() { if (_p1 && _p2 && _p3 && _p4) _ls('/dhikr/dhikr.js?v=20260618a', _done); }
  _ls('/dhikr/dua-data.js?v=20260326b',       function() { _p1 = true; _check(); });
  _ls('/dhikr/smart-dhikr.js?v=66',           function() { _p2 = true; _check(); });
  ...
  return dataP.then(function(json){
    ...
    return fontP.then(function(fontOk){
      ...
      if(font==='qcf4'&&!fontOk){
        ...
        var _ft=document.createElement('span');_ft.textContent=' '+(window.t?t('mushaf.font_offline'):'Ø³ØªØ§ÛŒÙ„Ø§ Ù¾Û•Ú•ÛŒÙ†ÛŽ Ù†Û•Ú©Û•ØªÛ• Ø¯Û•Ø³Øª Ø¨ÛŒ Ø¦ÛŒÙ†ØªÛ•Ø±Ù†Û•Øª.');_fb.appendChild(_ft);
        var _fr=document.createElement('button');_fr.className='mushaf-fallback-retry';_fr.textContent=window.t?t('general.retry','Ø¯ÙˆØ¨Ø§Ø±Û•'):'Ø¯ÙˆØ¨Ø§Ø±Û•';
...
function updateMushafProgress(view){
  ...
  function updateHeader(){
    ...
    // Header surah name â€” fades in smoothly when switching surahs
  ...
  // â”€â”€ Mark only ayahs whose top edge is on-screen (viewport-accurate tracker) â”€
