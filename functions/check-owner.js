// Migrated from Netlify to Cloudflare Pages
// Original: netlify/functions/check-owner.js

// Check if IP is owner (server-side only)
export async function onRequest(context) {
    const { request, env } = context;
    const headers = {
        'Access-Control-Allow-Origin': '*',
        'Access-Control-Allow-Headers': 'Content-Type',
        'Access-Control-Allow-Methods': 'GET, OPTIONS',
        'Content-Type': 'application/json'
    };

    if (request.method === 'OPTIONS') {
        return { statusCode: 200, headers, body: '' };
    }

    try {
        // Owner IPs stored securely on server
        const OWNER_IPS = [
            '185.136.148.162',
            '185.84.71.147',
            '185.136.148.130',
            '185.240.17.77'    // Phone
        ];

        // Get client IP from various headers (prioritize Cloudflare)
        const clientIP = Object.fromEntries(request.headers)['cf-connecting-ip'] ||           // Cloudflare real IP (highest priority)
                        Object.fromEntries(request.headers)['x-real-ip'] ||                  // Real IP header
                        Object.fromEntries(request.headers)['x-nf-client-connection-ip'] ||  // Netlify client IP
                        Object.fromEntries(request.headers)['x-forwarded-for']?.split(',')[0]?.trim() || // First IP in chain
                        Object.fromEntries(request.headers)['client-ip'];

        console.log('Check owner - Detected IP:', clientIP);

        const isOwner = OWNER_IPS.includes(clientIP);

        return {
            statusCode: 200,
            headers,
            body: JSON.stringify({
                isOwner,
                // Send owner IPs list only if requester is an owner (for device tracking)
                ownerIPs: isOwner ? OWNER_IPS : []
            })
        };
    } catch (error) {
        console.error('Check owner error:', error);
        return {
            statusCode: 500,
            headers,
            body: JSON.stringify({
                isOwner: false,
                error: 'Server error'
            })
        };
    }
};
