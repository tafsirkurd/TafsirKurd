...
// Original: netlify/functions/discord-interactions.js
...
export async function onRequest(context) {
    ...
    try {
        ...
        if (!PUBLIC_KEY) {
            ...
            return new Response(JSON.stringify({ error: 'Discord not configured' }), { status: 500, headers: { "Content-Type": "application/json" } });
        }

        // Verify Discord signature
        const signature = Object.fromEntries(request.headers)['x-signature-ed25519'];
        const timestamp = Object.fromEntries(request.headers)['x-signature-timestamp'];
        ...
        if (interaction.type === InteractionType.PING) {
            return new Response(JSON.stringify({ type: InteractionResponseType.PONG }), { status: 200, headers: { "Content-Type": "application/json" } });
        }
